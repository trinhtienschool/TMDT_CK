...
//                }
//                request.setAttribute("date_range", date_range);
                String supplier_id = String.valueOf((int) session.getAttribute("supplier_id"));
                if (request.getParameter("action") != null) {
                ...
                int commissionRevenue = Load_Supplier.sumOf("select SUM(o.total_price*o.commission_rate/100) from `order` o join supplier s on o.supplier_id = s.id where supplier_id= " + supplier_id + " and Month(o.date_created) = Month(CurDate())");
                ...
                int supplier_id_int = (int) session.getAttribute("supplier_id");
